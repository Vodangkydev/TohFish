...
@section('title', 'Rau Gia Vị - TOH fish')
...
<section class="products-section py-5">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-title text-center mb-5">Rau Gia Vị</h1>
                <p class="text-center text-muted mb-5">Rau gia vị tươi ngon từ TOH fish</p>
            </div>
        </div>
        <div class="row">
                ...
                <div class="col-lg-3 col-md-4 col-sm-6 mb-4">
                    <div class="product-card">
                        ...
                        <div class="product-info" style="margin-top: 12px;">
                            <h5 class="product-name"><a href="{{ route('product.detail', $product->images_id ?? ($index + 1)) }}" style="text-decoration: none; color: inherit;">{{ $product->content ?? 'Rau gia vị ' . ($index + 1) }}</a></h5>
                            ...
                            @endif
                            <p class="product-price">{{ number_format($product->price ?? 45000, 0, ',', '.') }}₫</p>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <form action="{{ route('cart.add') }}" method="POST" class="d-inline flex-grow-1">
                                    ...
                                    <input type="hidden" name="name" value="{{ $product->content ?? 'Rau gia vị ' . ($index + 1) }}">
                                    <input type="hidden" name="price" value="{{ $product->price ?? 45000 }}">
                ...
                <div class="product-card">
                    <div class="product-image" style="position: relative; width: 100%; aspect-ratio: 1 / 1; overflow: hidden; background-color: #f8f9fa;">
                        <a href="{{ route('product.detail', $i) }}" style="display: block; position: absolute; top: 0; left: 0; width: 100%; height: 100%;">
                            <img src="{{ asset('images/home/' . (($i % 14) + 1) . '.png') }}" alt="Rau gia vị {{ $i }}" class="img-fluid" style="width: 100%; height: 100%; object-fit: cover; display: block;">
                        </a>
                    </div>
                    <div class="product-info" style="margin-top: 12px;">
                        <h5 class="product-name"><a href="{{ route('product.detail', $i) }}" style="text-decoration: none; color: inherit;">Rau gia vị {{ $i }}</a></h5>
                        <p class="product-price">45,000₫</p>
                        <form action="{{ route('cart.add') }}" method="POST">
                            @csrf
                            <input type="hidden" name="product_id" value="{{ $i }}">
                            <input type="hidden" name="name" value="Rau gia vị {{ $i }}">
